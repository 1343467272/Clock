...
public class BreenoProxyProvider extends ContentProvider {
    ...
    /**
     * Writes the result as JSON so the LSPosed module (which invokes this provider as root
     * via `content call`) can reliably read back array values that the shell dump cannot show.
     */
    private void writeResultFile(Bundle out) {
        try {
            final java.io.File f = new java.io.File(mAppContext.getFilesDir(), "breeno_result.json");
            final java.io.FileOutputStream fos = new java.io.FileOutputStream(f);
            fos.write(bundleToJson(out).getBytes("UTF-8"));
            fos.close();
    ...
    static String bundleToJson(Bundle b) {
        ...
        for (String k : b.keySet()) {
            ...
            if (v == null) {
            ...
            } else if (v instanceof boolean[] a) {
                appendBoolArray(sb, a);
                ...
                for (int i = 0; i < a.length; i++) {
                    ...
                    sb.append('"').append(escapeJson(a[i] == null ? "" : a[i])).append('"');
                }
                sb.append(']');
            } else {
                sb.append("null");
    ...
    private static void appendLongArray(StringBuilder sb, long[] a) {
        sb.append('[');
        for (int i = 0; i < a.length; i++) {
            if (i > 0) {
                sb.append(',');
    ...
    private static void appendBoolArray(StringBuilder sb, boolean[] a) {
        sb.append('[');
        for (int i = 0; i < a.length; i++) {
            if (i > 0) {
                sb.append(',');
